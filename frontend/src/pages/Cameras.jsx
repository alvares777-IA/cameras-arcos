...
export default function Cameras() {
    ...
    const sortedAndFilteredCameras = [...cameras]
        .filter((cam) => {
            ...
            const getRes = () => {
                try { return parseRecursos(cam.recursos)?.resolucao || '' } catch { return '' }
            }
            return (
        ...
        .sort((a, b) => {
            ...
            switch (sortConfig.key) {
                ...
                case 'nome': valA = String(a.nome || '').toLowerCase(); valB = String(b.nome || '').toLowerCase(); break
                case 'url': valA = String(a.rtsp_url || '').toLowerCase(); valB = String(b.rtsp_url || '').toLowerCase(); break
                ...
                case 'recursos':
                    const resA = (() => { try { return parseRecursos(a.recursos)?.resolucao || '' } catch { return '' } })()
                    const resB = (() => { try { return parseRecursos(b.recursos)?.resolucao || '' } catch { return '' } })()
                    valA = String(resA).toLowerCase()
                    valB = String(resB).toLowerCase()
                    break
                default: return 0
            }
